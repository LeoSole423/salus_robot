...
unsigned char legacy_mask_cost_to_nav2_cost(unsigned char legacy_cost) {
  // Matches the Costmap2D constructor used by Nav2 Humble KeepoutFilter for
  // its OccupancyGrid mask: conceptual 0..100 is rounded onto 0..254.
  if (legacy_cost == 0) return 0;
  if (legacy_cost >= 100) return 254;
  return static_cast<unsigned char>(std::round(
    static_cast<double>(legacy_cost) * 254.0 / 100.0));
}
