...
void VectorKeepoutLayer::onInitialize() {
  ...
  profile_.halo_radius_m = std::max(0.0, profile_.halo_radius_m); profile_.halo_edge_cost = static_cast<unsigned char>(std::clamp(edge, 1, 99)); profile_.halo_min_cost = static_cast<unsigned char>(std::clamp(minimum, 1, edge));
  current_ = true;
...
void VectorKeepoutLayer::state_callback(const salus_interfaces::msg::ProjectedKeepoutState::SharedPtr msg) {
  ...
  std::lock_guard<std::mutex> lock(mutex_); if (msg->revision < revision_) return; previous_map_ = polygons_map_; polygons_map_ = std::move(next); revision_ = msg->revision; revision_dirty_ = true; current_ = true;
}
...
void VectorKeepoutLayer::add_bounds(const std::vector<Polygon> & polygons, double * min_x, double * min_y, double * max_x, double * max_y) const {
  for (const auto & p : polygons) { const auto b = p.bounds.expanded(profile_.halo_radius_m); *min_x = std::min(*min_x, b.min_x); *min_y = std::min(*min_y, b.min_y); *max_x = std::max(*max_x, b.max_x); *max_y = std::max(*max_y, b.max_y); }
}
void VectorKeepoutLayer::updateBounds(double, double, double, double * min_x, double * min_y, double * max_x, double * max_y) {
  ...
  add_bounds(current, min_x, min_y, max_x, max_y); add_bounds(old, min_x, min_y, max_x, max_y); add_bounds(rendered, min_x, min_y, max_x, max_y);
  { std::lock_guard<std::mutex> lock(mutex_); revision_dirty_ = false; previous_map_.clear(); previous_rendered_ = current; }
  current_ = true;
}
void VectorKeepoutLayer::updateCosts(nav2_costmap_2d::Costmap2D & master, int min_i, int min_j, int max_i, int max_j) {
  if (!enabled_ || min_i >= max_i || min_j >= max_j) return; std::vector<Polygon> polygons;
  { std::lock_guard<std::mutex> lock(mutex_); polygons = polygons_map_; }
  try { polygons = transform_polygons(polygons, layered_costmap_->getGlobalFrameID()); } catch (const tf2::TransformException &) { return; }
  ...
  for (unsigned int y = 0; y < spec.height; ++y) for (unsigned int x = 0; x < spec.width; ++x) { const auto cost = costs[y * spec.width + x]; if (cost > master.getCost(min_i + x, min_j + y)) master.setCost(min_i + x, min_j + y, cost); }
}
void VectorKeepoutLayer::reset() { std::lock_guard<std::mutex> lock(mutex_); previous_map_ = polygons_map_; revision_dirty_ = true; current_ = true; }
}  // namespace salus_navigation_costmap
