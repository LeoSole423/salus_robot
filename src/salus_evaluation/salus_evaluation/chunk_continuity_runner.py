...
APPROACH_25_POLICY = "approach_25"
BASE_LAT = -31.4858037
BASE_LON = -64.2410570
TURN_RADIUS_M = 8.0
...
def _trajectory_metrics(samples, target_xy, target_yaw=None):
    """Measure the observed odometry path without treating it as the plan."""
    if not samples:
        return {
            "available": False,
            "sample_count": 0,
    ...
    final_error = math.hypot(
        samples[-1]["x_m"] - target_xy[0],
        samples[-1]["y_m"] - target_xy[1],
    )
    final_yaw_error = None
    if target_yaw is not None:
        final_yaw_error = angle_delta(samples[-1]["yaw_rad"], target_yaw)
    return {
        "available": True,
...
class ChunkContinuityRunner(Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(NavPath, "/received_global_plan",
                                 self._received_global_plan, 20)
    ...
    def _received_global_plan(self, message):
        points = _point_path(message)
        if points:
            self.received_global_plans.append({
                "stamp_s": _stamp(message), "points": points,
            })
    ...
    def _final_command(self, message):
        """Record a command with receipt time; CmdVelFinal has no header."""
    ...
    def _request(self, points):
        ...
        if self.policy == APPROACH_25_POLICY and len(points) >= 3:
            incoming = math.atan2(
            ...
            request.yaws_deg[1] = math.degrees(
                incoming + 0.25 * angle_delta(outgoing, incoming)
            )
    ...
    def _wait_for_completion(self, timeout_s=240.0):
        terminal = {"COMPLETED"}
        failed = {"ABORTED", "CANCELLED", "PAUSED", "BLOCKED"}
        deadline = time.monotonic() + timeout_s
        while time.monotonic() < deadline:
            self.last_state = self._state()
            status = str(self.last_state.status)
            if status in terminal:
                return self.last_state
            if status in failed:
                raise RuntimeError(
                    f"route did not complete: status={status} "
                    f"reason={self.last_state.blocked_reason_text!r}"
                )
            rclpy.spin_once(self, timeout_sec=0.1)
        raise TimeoutError("productive route_executor did not reach COMPLETED")
    ...
    def _summary(self, origin):
        ...
        for window in windows:
            start_s = window["dispatch_stamp_s"]
            end_s = window["result_stamp_s"] or float("inf")
            window["received_global_plans"] = [
                item for item in self.received_global_plans
                if start_s <= item["stamp_s"] < end_s
            ]
            if window["goal_accepted"] is not None:
                window["executed_odometry"] = [
                    item for item in self.odom
                    if window["goal_accepted"]["stamp_s"]
                    <= item["stamp_s"] <= end_s
                ]
                poses_xy = window["dispatch"].get("poses_xy") or []
                yaws_deg = window["dispatch"].get("yaws_deg") or []
                target_yaw = None
                if yaws_deg:
                    try:
                        candidate = math.radians(float(yaws_deg[-1]))
                        if math.isfinite(candidate):
                            target_yaw = candidate
                    except (TypeError, ValueError):
                        target_yaw = None
                window["executed_trajectory"] = _trajectory_metrics(
                    window["executed_odometry"],
                    tuple(poses_xy[-1]) if poses_xy else (0.0, 0.0),
                    target_yaw,
                )
        geometry_quality = [
            {
        ...
        summary = {
            ...
            "terminal_status": str(self.last_state.status),
            "errors": [],
            "route_executor": {
            ...
            "plans": self.plans,
            "received_global_plans": self.received_global_plans,
            "controller_status": self.controller_status,
            "final_commands": [
                {
                    "stamp_s": item["stamp_s"],
                    "linear_x": float(item["message"].twist.linear.x),
                    "angular_z": float(item["message"].twist.angular.z),
                    "source": int(item["message"].source),
                    "brake_pct": int(item["message"].brake_pct),
                }
            ...
            "observed_metrics": {
                ...
                "final_auto_commands": sum(
                    int(item["message"].source) == CmdVelFinal.SOURCE_AUTO
                    for item in self.final_commands
                ),
                "final_brake_samples": sum(
                    int(item["message"].brake_pct) > 0 for item in self.final_commands
                ),
            },
        }
        manifest = {
