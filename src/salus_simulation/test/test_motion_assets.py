...
def test_empty_world_has_only_motion_required_systems() -> None:
...
def test_motion_bridge_preserves_control_and_feedback_endpoints() -> None:
    ...
    for endpoint in ("/cmd_vel_gazebo", "/cmd_vel_steer", "/odom_raw", "/odom", "/clock"):
        assert endpoint in bridge
    assert "ROS_TO_GZ" in bridge
    assert bridge.count("GZ_TO_ROS") == 2
