...
def main():
    ...
    try:
        ...
        wait(n,lambda:n.global_maps and has_core(n.global_maps[-1],*zone_a),"zone A core not rasterized")
        ...
        moved={"type":"FeatureCollection","features":[square("zone_a",*zone_a),square("zone_b",FAR_X+4.),square("zone_c",FAR_C_X)]}; move_revision=n.projected[-1].revision; moved_response=call(n,n.set_zones,SetZonesGeoJson.Request(geojson=json.dumps(moved)),"move zone unavailable")
        ...
        remove_revision=n.projected[-1].revision; removed=call(n,n.set_zones,SetZonesGeoJson.Request(geojson=json.dumps({"type":"FeatureCollection","features":[square("zone_a",*zone_a),square("zone_c",FAR_C_X)]})),"remove zone unavailable")
        ...
        zone_c=local_to_map(n.odom[-1],9.,0.); third_revision=n.projected[-1].revision
        third={"type":"FeatureCollection","features":[square("zone_a",*zone_a),square("zone_c",*zone_c)]}; third_response=call(n,n.set_zones,SetZonesGeoJson.Request(geojson=json.dumps(third)),"set zone C unavailable")
        if not third_response.ok: raise RuntimeError(third_response.error)
        wait(n,lambda:n.projected and n.projected[-1].revision>third_revision and any(p.zone_id=="zone_c" for p in n.projected[-1].polygons),"zone C revision unavailable")
        zone_c=polygon_center(next(p for p in n.projected[-1].polygons if p.zone_id=="zone_c"))
